// Intended path: src/components/admin/hr/CheckInsOverTimeChart.tsx
// Same dependency on the unfiltered-attendance-list assumption as
// AttendanceByStatusChart — see that file's header comment.
  ...
  return (
    <div className="bg-panel border border-hairline rounded-lg p-4 shadow-elevation-1">
      <h3 className="text-sm font-medium text-ink-primary mb-3">
        {t('hr.overview.checkInsOverTime.title')}
      </h3>
      ...
      {isLoading ? (
        ...
        <EmptyState
          title={t('hr.overview.checkInsOverTime.emptyTitle')}
          description={
            isError
              ? t('hr.overview.checkInsOverTime.errorDescription')
              : t(
                'hr.overview.checkInsOverTime.emptyDescription'
              )
          }
        />
      ) : (
            ...
            <Tooltip
              ...
              formatter={(value) => [
                typeof value === 'number'
                  ? value
                  : Number(value ?? 0),
                t('hr.overview.checkInsOverTime.countLabel'),
              ]}
            />
