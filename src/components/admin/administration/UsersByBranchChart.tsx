// Intended path: src/components/admin/administration/UsersByBranchChart.tsx
//
// UserResponse already includes branchName directly — no client-side
// lookup against the Branches module needed, unlike most grouping charts
// in the other Overview pages.
...
export function UsersByBranchChart({
  data,
  isLoading,
}: Props) {
  ...
  return (
    <div className="bg-panel border border-hairline rounded-lg p-4 shadow-elevation-1">
      <h3 className="text-sm font-medium text-ink-primary mb-3">
        {t(
          'administration.overview.usersByBranch.title'
        )}
      </h3>
      ...
      {isLoading ? (
        ...
        <EmptyState
          title={t(
            'administration.overview.usersByBranch.emptyTitle'
          )}
          description={t(
            'administration.overview.usersByBranch.emptyDescription'
          )}
        />
      ) : (
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(value) => [
                value,
                t(
                  'administration.overview.usersByBranch.countLabel'
                ),
              ]}
            />
